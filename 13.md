...
คำตอบ: CVE-2017-15945 (บทที่ 3 หัวข้อ 3.3 หัวข้อย่อย 3.3.6)

# Step 1

นี่คือคำสั่งที่เราจะใช้ เอามาจากคอร์ส Cisco เลย เปลี่ยนแค่ IP จะไม่บอกให้กลับไปหาเองในคอร์ส เพราะมันเยอะ แต่เดี๋ยวจะอธิบายคำสั่งให้ + ถ้าอยากกลับไปดูเองจริง ๆ ดูจากในวงเล็บหลังคำตอบที่เกรซแปะไว้ได้เลย  (บทที่ 3 หัวข้อ 3.3 หัวข้อย่อย 3.3.6)

```
nmap -sV --script vulners --script-args mincvss=4 10.6.6.14
```
อธิบายคำสั่ง

1. nmap คือเครื่องมือที่ใช้สำหรับสแกนเครือข่ายและตรวจสอบความปลอดภัย
2. -sV สแกนเพื่อตรวจสอบหาชื่อซอฟต์แวร์และเลขเวอร์ชันบนพอร์ตที่เปิดอยู่
3. --script vulners เป็นการเรียกใช้สคริปต์ NSE ที่ชื่อ vulners เพื่อนำข้อมูลเวอร์ชันที่ได้ไปเทียบกับฐานข้อมูลช่องโหว่ (CVE) ออนไลน์ การทำงานของมันคือ เมื่อมันได้ชื่อบริการและเวอร์ชันมาจากคำสั่ง -sV แล้ว สคริปต์นี้จะนำชื่อและเวอร์ชันนั้นไปค้นหาในฐานข้อมูลออนไลน์ทันทีว่า ซอฟต์แวร์เวอร์ชันนี้ มีประวัติช่องโหว่รหัส CVE อะไรที่ถูกประกาศไว้บ้าง แล้วดึงข้อมูลนั้นกลับมาแสดงผล
4. --script-args คือการส่งค่าตัวแปรเสริม (Arguments) เข้าไปควบคุมการทำงานภายในสคริปต์
5.mincvss=4 ย่อมาจาก Minimum CVSS Score = 4 ค่า CVSS (Common Vulnerability Scoring System) คือคะแนนความรุนแรงของช่องโหว่ มีสเกลตั้งแต่ 0.0 ถึง 10.0 .ในส่วนนี้บอกว่า ให้สคริปต์คัดกรองและแสดงผลเฉพาะช่องโหว่ที่มีคะแนนความรุนแรงตั้งแต่ระดับ 4.0 ขึ้นไปเท่านั้น ช่องโหว่ไหนที่คะแนนต่ำกว่า 4 ไม่ต้องนำมาแสดงบนหน้าจอ เพื่อป้องกันไม่ให้ข้อมูลรายงานรกเกินไป

ผลลัพธ์ (เอามาเต็ม ๆ เลย ไม่ย่อ ไม่ตัด แนะนำว่าให้เลื่อนลงไปด้านล่างสุดเลย เพราะคำตอบอยู่ที่นั่น)
```
┌──(root㉿Kali)-[/home/kali]
└─# nmap -sV --script vulners --script-args mincvss=4 10.6.6.14
Starting Nmap 7.94 ( https://nmap.org ) at 2026-07-01 00:34 UTC
...
| vulners: 
|   cpe:/a:mysql:mysql:5.5.60-0ubuntu0.14.04.1: 
|_      CVE-2017-15945  7.8     https://vulners.com/cve/CVE-2017-15945
...
Q: แล้วที่ยาว ๆ ก่อนหน้านี้คืออะไร และทำไมเรามาโฟกัสแค่ส่วนท้ายสุด

A: ข้อความยาว ๆ ทั้งหมดก่อนหน้าพอร์ต 3306 คือ คลังข้อมูลสิ่งแปลกปลอมความเสี่ยงที่ Nmap ตรวจพบเฉพาะบนพอร์ต 80 (Apache 2.4.7) แสดงให้เห็นว่า บนช่องทางเว็บนี้ช่องทางเดียว มีช่องโหว่ความปลอดภัยสะสมมาตั้งแต่ปีอดีตจนถึงปีปัจจุบัน นับร้อยรายการมีคะแนนความรุนแรงอยู่ในเกณฑ์ Medium ถึง Critical และบางช่องโหว่มีโค้ดโจมตีสำเร็จรูป (*EXPLOIT*) เผยแพร่อยู่บน GitHub และฐานข้อมูลสาธารณะ

A VIP: ระบบตรวจพบพอร์ตที่เปิดให้บริการหลัก ๆ 2 พอร์ต ดังนี้

1. 80/tcp open http Apache httpd 2.4.7 ((Ubuntu))

   อธิบาย: เป้าหมายเปิดเว็บเซิร์ฟเวอร์ด้วยซอฟต์แวร์ Apache httpd เวอร์ชัน 2.4.7 บนระบบปฏิบัติการ Ubuntu

   สิ่งที่ต้องโฟกัส: มีรายชื่อรหัสช่องโหว่ CVE ยาวต่อเนื่องกันหลายช่องโหว่ ตรวจพบรหัสช่องโหว่ใหม่และเก่าที่มีคะแนนความรุนแรง สูงถึง 9.8 และ 10.0 เป็นจำนวนมาก (ถ้ามีระบุว่า *EXPLOIT* แปลว่าช่องโหว่เหล่านี้มีโค้ดสำหรับใช้ยิงเผยแพร่อยู่บนอินเทอร์เน็ตแล้ว)

3. 3306/tcp open mysql MySQL 5.5.60-0ubuntu0.14.04.1

   อธิบาย: ระบบรันระบบจัดการฐานข้อมูล MySQL เวอร์ชัน 5.5.60 บน Ubuntu 14.04

   สิ่งที่ต้องโฟกัส: ในพอร์ตนี้ตรวจพบช่องโหว่ที่ตรงล็อกเพียงตัวเดียวคือ CVE-2017-15945 (7.8) ซึ่งอยู่ในระดับความรุนแรงสูง

   ```
   3306/tcp open mysql MySQL 5.5.60-0ubuntu0.14.04.1
   ```

   3306/tcp: พอร์ตหมายเลข 3306 เป็นพอร์ตที่ใช้สำหรับบริการระบบจัดการฐานข้อมูล MySQL โดยรันบนโปรโตคอลแบบ TCP

   open: สถานะของพอร์ตเป็นเปิด หมายความว่าเครื่องเซิร์ฟเวอร์เป้าหมายยอมรับการเชื่อมต่อจากภายนอกวิ่งเข้ามาที่ฐานข้อมูลนี้

   mysql: บริการที่ตรวจพบ

   MySQL 5.5.60-0ubuntu0.14.04.1: เซิร์ฟเวอร์นี้ใช้ระบบฐานข้อมูล MySQL เวอร์ชัน 5.5.60 ซึ่งติดตั้งอยู่บนระบบปฏิบัติการ Ubuntu เวอร์ชัน 14.04
   ...
   CVE-2017-15945: เป็นรหัสช่องโหว่ที่ถูกค้นพบและบันทึกในระบบฐานข้อมูลในปี ค.ศ. 2017

   7.8: คะแนนความรุนแรงตามมาตรวัด CVSS Score ซึ่งคะแนน 7.8 จัดอยู่ในระดับสูง

   MAC Address: 02:42:0A:06:06:0E (Unknown) ย่อมาจาก Media Access Control Address เป็นหมายเลขประจำตัวทางกายภาพที่ถูกฝังมากับการ์ดเครือข่ายของเครื่องเซิร์ฟเวอร์นั้น ๆ ความพิเศษคือ ในกรณีที่ขึ้นว่า Unknown ร่วมกับหมายเลขที่ขึ้นต้นด้วย 02:42:...หมายความว่า เซิร์ฟเวอร์เครื่องนี้กำลังทำงานอยู่บนสภาพแวดล้อมจำลอง
